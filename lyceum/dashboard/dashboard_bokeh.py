...
def dashboard_bokeh_components(filters=None):
    ...
    if df.empty:
        ...
        if filters and filters.get('students_sort'):
            if filters['students_sort'] == 'asc':
                df = df.sort_values(by='student_count', ascending=True)
            elif filters['students_sort'] == 'desc':
                df = df.sort_values(by='student_count', ascending=False)
    ...
    if df.empty:
        ...
        source = ColumnDataSource(df)
        p = figure(x_range=df['surname'].tolist(),
                   title="Пропущені уроки студентами", height=400)
        p.vbar(x='surname', top='missed_lessons', width=0.9,
               source=source, color="#e84d60")
        p.xgrid.grid_line_color = None
        p.y_range.start = 0
    ...
    if df.empty:
        ...
        source = ColumnDataSource(df)
        p = figure(x_range=df['surname'].tolist(),
                   title="Батьки з пільговими дітьми", height=400)
        p.vbar(x='surname', top='privileged_children', width=0.9,
               source=source, color="#66c2a5")
        p.xgrid.grid_line_color = None
        p.y_range.start = 0
    ...
    if df.empty:
        ...
        source = ColumnDataSource(df)
        p = figure(x_range=df['name'].tolist(),
                   title="Привілейовані студенти по формах", height=400)
        p.vbar(x='name', top='privileged_students', width=0.9,
               source=source, color="#ddb7b1")
        p.xgrid.grid_line_color = None
        p.y_range.start = 0
    ...
    if df.empty:
        ...
        source = ColumnDataSource(df)
        p = figure(x_range=df['surname'].tolist(),
                   title="Кількість книг на студента", height=400)
        p.vbar(x='surname', top='books_count', width=0.9,
               source=source, color="#c9d9d3")
        p.xgrid.grid_line_color = None
        p.y_range.start = 0
    ...
    if df.empty:
        ...
        source = ColumnDataSource(df)
        p = figure(title="Кількість ризикових студентів по днях",
